...
class STA_LTA_Processor:
    def __init__(self, sampling_rate=6.625):
        self.sampling_rate = (
            sampling_rate  # Default from the notebook's miniseed file info
        )
    ...
    def process(self, data, sta_len=120, lta_len=600):
        ...
        cft = classic_sta_lta(
            data, int(sta_len * df), int(lta_len * df)
        )  # Calculate STA/LTA
        return cft
            ...
            for filename in os.listdir(test_data_dir):
                if filename.endswith(".mseed"):
                    ...
                    try:
                        ...
                        predicted_time_rel = tr_times[
                            on_off[0][0]
                        ]  # Get the relative time in seconds
                        predicted_time_abs = tr.stats.starttime + timedelta(
                            seconds=predicted_time_rel
                        )

                        print(
                            f"Detected Event Arrival Time (relative): {predicted_time_rel:.2f} s, (absolute): {predicted_time_abs}"
                        )
                        ...
                        self.save_plot(
                            tr_times, tr_data, predicted_time_rel, base_filename
                        )
