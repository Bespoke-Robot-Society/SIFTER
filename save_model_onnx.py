...
from cnn_model import SpectrogramArrivalCNN  # Import your model
...
def load_real_data(data_dir, batch_size=1):
    ...
    preprocessor = Preprocessing(save_dir=None)  # Update this with actual save_dir if needed
    spectrogram_data, _ = preprocessor.preprocess_martian_data(data_dir)  # Preprocess your data
    ...
    real_data = real_data.unsqueeze(1)  # Add the channel dimension for a single channel (1, H, W)
...
def save_pytorch_model_to_onnx(model_path, onnx_model_path, real_input, opset_version=11):
    """
