...
class Preprocessing:
    ...
    def preprocess_data(self, file_path, filename, arrival_time_abs=None, combine_images=True):
        ...
        try:
            ...
            arrival_time_rel = self.detect_event(filtered_trace, nsta=int(sampling_rate * 1), nlta=int(sampling_rate * 20))
            if arrival_time_rel is not None:
                arrival_time_abs = self.convert_rel_to_abs_time(arrival_time_rel, starttime)
                print(f"Detected Event Arrival Time (relative): {arrival_time_rel} s, (absolute): {arrival_time_abs}")
    ...
    def preprocess_lunar_data(self, catalog, data_dir, combine_images=True):
        ...
        for idx, row in catalog.iterrows():
            ...
            if os.path.exists(file_path):
                ...
                spectrogram_image_path = self.preprocess_data(file_path, filename, arrival_time_abs=arrival_time_abs, combine_images=combine_images)
    ...
    def preprocess_martian_data(self, data_dir, combine_images=True):
        ...
        for file in os.listdir(data_dir):
            if file.endswith(".mseed"):
                ...
                spectrogram_image_path = self.preprocess_data(file_path, file, combine_images=combine_images)
