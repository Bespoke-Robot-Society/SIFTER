...
def predict_arrival(cnn_model, preprocessed_data, device):
    with torch.no_grad():
        ...
        data_tensor = torch.tensor(preprocessed_data, dtype=torch.float32).unsqueeze(0).unsqueeze(0)
        ...
        for file in os.listdir(test_data_dir):
            if file.endswith(".mseed"):
                ...
                try:
                    ...
                    # Predict arrival time
                    predicted_arrival_time_rel = predict_arrival(model, Sxx, device)

                    # Convert to absolute time (assuming the start time is known)
                    tr = read(file_path)[0]
                    starttime = tr.stats.starttime.datetime
                    predicted_arrival_time_abs = starttime + timedelta(seconds=predicted_arrival_time_rel)
